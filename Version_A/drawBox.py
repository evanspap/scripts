...
#
# AUTHOR: Jason Vertrees, Evangelos Papadopoulos
# DATE  : 2/20/2009
# NOTES : See comments below.
# example:
# drawBox(onoma="P2", x=24.85866667, y=17.09, z= 8.3, sx=16.0, sy=16.0, sz=16.0)
#############################################################################
def drawBox(onoma="", x=0.0, y=0.0, z=0.0, sx=1.0,sy=1.0,sz=1.0, lineWidth=2.0, r=1.0, g=1.0, b=1.0):
    """
    DESCRIPTION
...
#    ([minX, minY, minZ],[maxX, maxY, maxZ]) = cmd.get_extent(sel)
    ...
    minX=float(x)-float(sx)/2
    minY=float(y)-float(sy)/2
    minZ=float(z)-float(sz)/2
    maxX=float(x)+float(sx)/2
    maxY=float(y)+float(sy)/2
    maxZ=float(z)+float(sz)/2
