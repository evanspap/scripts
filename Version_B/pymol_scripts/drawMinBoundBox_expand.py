...
#
#############################################################################
def drawMinBoundingBoxexp(sel="(all)", x=0.0, y=0.0, z=0.0, lineWidth=2.0, r=1.0, g=1.0, b=1.0):
    """
    DESCRIPTION
        Given sel, draw the minimum bounding box around it.

    USAGE:
        drawMinBoundingBoxexp [sel, [lineWidth, [r, [g, b]]]]

    PARAMETERS:
        sel,            the selection to enboxen.  :-)
                        defaults to (all)
        x, y, z,        respective axes expansion in A
    ...
    curName = "minBB_" + str(sel)+"_exp"


    cmd.load_cgo(minBB,curName)


cmd.extend('drawMinBoundingBoxexp', drawMinBoundingBoxexp)